#include "demobot_hardware.hpp"

namespace demo_hardware
{

DemobotHardware::DemobotHardware()
    : logger_(rclcpp::get_logger("DemobotHardware"))
{}

CallbackReturn DemobotHardware::on_init(const hardware_interface::HardwareInfo & info)
{
...
std::vector<hardware_interface::StateInterface> DemobotHardware::export_state_interfaces()
{
...
std::vector<hardware_interface::CommandInterface> DemobotHardware::export_command_interfaces()
{
...
CallbackReturn DemobotHardware::on_configure(const rclcpp_lifecycle::State & /*previous_state*/)
{
    ...
    wiringPiISR(LEFT_WHL_ENC_INT, INT_EDGE_RISING, left_wheel_pulse);
    wiringPiISR(RIGHT_WHL_ENC_INT, INT_EDGE_RISING, right_wheel_pulse);
...
CallbackReturn DemobotHardware::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
{
...
CallbackReturn DemobotHardware::on_deactivate(const rclcpp_lifecycle::State & /*previous_state*/)
{   
...
return_type DemobotHardware::read(const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
    // Obtain elapsed time
...
return_type DemobotHardware::write(const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{   
    ...
    // RCLCPP_INFO(logger_, "write: left = %.2lf, right = %.2lf", left_motor_counts_per_loop, right_motor_counts_per_loop);
